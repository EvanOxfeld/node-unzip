...
Parse.prototype.close = function() {
  var self = this;
  
  self.end();
  
  self._pullStream.end(function() {
    self._pullStream.unpipe();
    self._pullStream.pull(function(err, data){}); //this drains the rest  
  });
...
Parse.prototype._transform = function (chunk, encoding, callback) {
  if(this._closing) {
    return callback(new Error("Can't write to closed stream. (Did you forget to unpipe?)"));  
  }
